...
;; "
;;   (let* ((len (ats-array-partials array))
...
copy an array of arrays
"
  (let* ((partials (ats-array-partials array))
         (frames (ats-array-frames array))
         (new (make-ats-array partials frames)))
    (dotimes (p partials)
      (dotimes (f frames)
        (setf (ats-aref new p f) (ats-aref array p f))))
    new))	
