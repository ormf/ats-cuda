...
(defun tracker (file snd &key 	
		     ...
		     (force-window NIL))
  (let* (;;; input file
	 (fil (incudine::buffer-load (concatenate 'string *ats-snd-dir* file)))
         ;;; ATS sound
	 ...
	 (nd (if duration (+ st (floor (* duration file-sampling-rate))) (sound-framples fil)))
	 ;;; number of samples to read
	 (total-samps (- nd st))
         ;;; file duration
	 (file-duration (dfloat (/ total-samps file-sampling-rate)))
	 ...
	 (fft-struct (make-ats-fft :size N
				   ...
				   :fdr (make-double-float-array N :initial-element 0.0d0)
				   :fdi (make-double-float-array N :initial-element 0.0d0)))
 	 ;;; window array
	 ...
	 (filptr (- st M-over-2))
	 ;;; minimum SMR 
	 (min-smr (or SMR-threshold 0.0))
	 (n-partials 0)
	 (tracks nil)
	 (peaks nil)
	 (unmatched-peaks nil)
         (fft-instance (incudine.analysis::make-fft n :window-function
                                 (symbol-function
                                  (intern (symbol-name window-type)
                                          :incudine.gen)))))
	...
	;;; multiply by window
      (dotimes (k M)
          (when (>= filptr 0)
	    (setf (incudine:smp-ref (incudine.analysis:analysis-input-buffer fft-instance)
                         (ash (mod (+ k first-point) N) 1)) 
		  (incudine:smp-ref (incudine:buffer-data fil) filptr)))
        (incf filptr))
      ...
      (setf (aref win-samps frame-n) smp)
        ;;; set timer
      (setf tmp (dfloat (/ (- smp st) file-sampling-rate)))
        ;;; get the dft
      (incudine.analysis:compute-fft fft-instance)
      (incudine.analysis::fft->r-i-arrays
       fft-instance
	...
	(if (and (> frame-n 0) 
		 (setf tracks
                       (update-tracks tracks track-length frame-n ana-frames last-peak-contribution)))
	    (let ((cpy-peak nil))
	...
	(let ((pe (find k (aref ana-frames frame) :key #'ats-peak-track)))
	  (if pe
	      (setf (aref (aref (ats-sound-amp sound) k) frame)(dfloat (ats-peak-amp pe))
		    (aref (aref (ats-sound-frq sound) k) frame)(dfloat (ats-peak-frq pe))
		    (aref (aref (ats-sound-pha sound) k) frame)(dfloat (ats-peak-pha pe))))
	  ;;; set time anyways
	  (setf (aref (aref (ats-sound-time sound) k) frame)
		(dfloat (/ (- (aref win-samps frame) st) file-sampling-rate))))))
